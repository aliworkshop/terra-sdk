...
If you want to open a PR in Cosmos SDK to update the documentation, please follow the guidelines in [`CONTRIBUTING.md`](https://github.com/aliworkshop/terra-sdk/tree/master/CONTRIBUTING.md#updating-documentation).
...
- Each `docs/<locale>/` folder must follow the same folder structure within `docs/`, but only content in the following folders needs to be translated and included in the respective `docs/<locale>/` folder:
    ...
    - `docs/migrations/`
    - `docs/run-node/`
- Each `docs/<locale>/` folder must also have a `README.md` that includes a translated version of both the layout and content within the root-level [`README.md`](https://github.com/aliworkshop/terra-sdk/tree/master/docs/README.md). The layout defined in the `README.md` is used to build the homepage.
...
Then, edit the `swagger.yaml` manually; it is found [here](https://github.com/aliworkshop/terra-sdk/blob/master/client/lcd/swagger-ui/swagger.yaml)
